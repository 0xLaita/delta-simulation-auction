...
class TokenStorageSlotsCache {
  private static TOKEN_STORAGE_SLOTS: Record<number, Record<string, TokenStorageSlots>> | null = null;
  private static readonly TOKEN_STORAGE_SLOTS_FILEPATH = path.join(__dirname, "token-storage-slots.json");

  private static async loadTokenStorageSlots(): Promise<Record<number, Record<string, TokenStorageSlots>>> {
    return JSON.parse(
  ...
  static async getTokenStorageSlots(chainId: number, token: string): Promise<TokenStorageSlots | null> {
    if (!TokenStorageSlotsCache.TOKEN_STORAGE_SLOTS) {
      TokenStorageSlotsCache.TOKEN_STORAGE_SLOTS = await TokenStorageSlotsCache.loadTokenStorageSlots();
    }

    return TokenStorageSlotsCache.TOKEN_STORAGE_SLOTS[chainId]?.[token.toLowerCase()] ?? null;
  }

  static async setTokenStorageSlots(chainId: number, token: string, slots: TokenStorageSlots): Promise<void> {
    if (!TokenStorageSlotsCache.TOKEN_STORAGE_SLOTS) {
      TokenStorageSlotsCache.TOKEN_STORAGE_SLOTS = await TokenStorageSlotsCache.loadTokenStorageSlots();
    }
    ...
    TokenStorageSlotsCache.TOKEN_STORAGE_SLOTS[chainId][token.toLowerCase()] = slots;
...
export class TenderlySimulator {
  ...
  public async simulateTransaction(request: SimulateTransactionRequest): Promise<Simulation> {
    const data = {
  ...
  public async getSimulatedTransactionDetails(id: string): Promise<SimulatedTransactionDetails | null> {
    try {
      const { data } = await axios.post(
        `https://api.tenderly.co/api/v1/account/${ACCOUNT_SLUG}/project/${PROJECT_SLUG}/simulations/${id}`,
        {},
        { headers: { "X-Access-Key": ACCESS_TOKEN } },
      );
  ...
  getSLOADCalls = (callTrace: SimulatedTransactionCall): SimulatedTransactionCall[] => {
   ...
   */
  calculateAddressBalanceSlot(balanceOfSlot: string, owner: string, isVyper = false) {
    return isVyper
  ...
  calculateSolidityAddressBalanceSlot(balanceOfSlot: string, owner: string) {
    return ethers.keccak256(ethers.concat([ethers.zeroPadValue(owner, 32), balanceOfSlot]));
  }
  ...
  calculateVyperAddressBalanceSlot(balanceOfSlot: string, owner: string) {
    return ethers.keccak256(ethers.concat([balanceOfSlot, ethers.zeroPadValue(owner, 32)]));
  }
   ...
   * @param isVyper `true` if contract is written in Vyper
   */
  calculateAddressAllowanceSlot(allowanceSlot: string, owner: string, spender: string, isVyper = false) {
    return isVyper
      ? this.calculateVyperAddressAllowanceSlot(allowanceSlot, owner, spender)
      : this.calculateSolidityAddressAllowanceSlot(allowanceSlot, owner, spender);
  }
   ...
   * @param spender spender's address
   */
  calculateSolidityAddressAllowanceSlot(allowanceSlot: string, owner: string, spender: string) {
    const slotHash = ethers.keccak256(
      ethers.concat([ethers.zeroPadValue(owner, 32), ethers.zeroPadValue(allowanceSlot, 32)]),
    );
   ...
   * @param spender spender's address
   */
  calculateVyperAddressAllowanceSlot(allowanceSlot: string, owner: string, spender: string) {
    const slotHash = ethers.keccak256(
      ethers.concat([ethers.zeroPadValue(allowanceSlot, 32), ethers.zeroPadValue(owner, 32)]),
    );

    return ethers.keccak256(ethers.concat([slotHash, ethers.zeroPadValue(spender, 32)]));
  }

  buildBalanceOfSimulationRequest(chainId: number, token: string, owner: string): SimulateTransactionRequest {
    const iface = new ethers.Interface(["function balanceOf(address owner) view returns (uint)"]);
  ...
  buildAllowanceSimulationRequest(
    ...
    const iface = new ethers.Interface(["function allowance(address owner, address spender) view returns (uint)"]);
   ...
   */
  async findTokenBalanceOfSlot(chainId: number, token: string): Promise<{ slot: string; isVyper?: boolean }> {
    const account = TenderlySimulator.DEFAULT_OWNER;

    const balanceOfSimulationRequest = this.buildBalanceOfSimulationRequest(chainId, token, account);

    const { id: simulationId } = await this.simulateTransaction(balanceOfSimulationRequest);

    const simulationDetails = await this.getSimulatedTransactionDetails(simulationId);
    ...
    const readSlots = sloadCalls.map((call) => call.storage_slot?.[0]).filter<string>((slot): slot is string => !!slot);
    ...
    for (const startingPoint of startingPoints) {
      for (let i = startingPoint; i < startingPoint + 1_000n; i += 1n) {
        const candidateSlot = ethers.AbiCoder.defaultAbiCoder().encode(["uint"], [i]);
        // try solidity slot
        const solitidyBalanceOfSlot = this.calculateSolidityAddressBalanceSlot(candidateSlot, account);
        if (readSlots.includes(solitidyBalanceOfSlot)) {
          return { slot: candidateSlot };
        }
        // try vyper slot
        const vyperBalanceOfSlot = this.calculateVyperAddressBalanceSlot(candidateSlot, account);
    ...
    throw new Error(`Could not find a 'balanceOf' mapping slot for token ${token} on chain ${chainId}`);
  }
   ...
   * @param token token address
   */
  async findTokenAllowanceSlot(chainId: number, token: string): Promise<{ slot: string; isVyper?: boolean }> {
    const account = TenderlySimulator.DEFAULT_OWNER;
    const spender = TenderlySimulator.DEFAULT_SPENDER;

    const allowanceSimulationRequest = this.buildAllowanceSimulationRequest(chainId, token, account, spender);

    const { id: simulationId } = await this.simulateTransaction(allowanceSimulationRequest);
    ...
    for (const init of startingPoints) {
      for (let i = init; i < init + 1000n; i += 1n) {
        const candidateSlot = ethers.AbiCoder.defaultAbiCoder().encode(["uint"], [i]);
        // try solidity
        const solidityAllowanceSlot = this.calculateSolidityAddressAllowanceSlot(candidateSlot, account, spender);
        if (readSlots.includes(solidityAllowanceSlot)) {
          return { slot: candidateSlot };
        }
        // try vyper
        const vyperAllowanceSlot = this.calculateVyperAddressAllowanceSlot(candidateSlot, account, spender);
    ...
    throw new Error(`Could not find a 'allowance' mapping slot for token ${token} on chain ${chainId}`);
  }
   ...
   */
  async getTokenStorageSlots(chainId: number, token: string): Promise<TokenStorageSlots> {
    ...
    // check cache
    const cachedSlots = await TokenStorageSlotsCache.getTokenStorageSlots(chainId, normalizedToken);
  ...
  async addTokenBalanceOverride(
    ...
    const slotToOverride = this.calculateAddressBalanceSlot(tokenSlots.balanceSlot, account, tokenSlots.isVyper);
    // add the balance override
    stateOverride[token] ||= {};

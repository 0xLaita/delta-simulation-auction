...
describe("Tenderly", () => {
  ...
  describe("calculateAddressBalanceSlot", () => {
    it("should calculate account balance storage slot", () => {
      const balanceOfSlot = ethers.AbiCoder.defaultAbiCoder().encode(["uint"], [9]); // usdc's `balanceOf` mapping slot
      // https://dashboard.tenderly.co/paraswap/paraswap/simulator/d7964910-d8ce-446b-a347-c4f1da1acead/debugger?trace=0.2.0.0
      const expected = "0x8cef03c2b7574a38d9a9e8dd69980240d41d70485643f407de33055936c48f97";
      const calculated = tenderly.calculateAddressBalanceSlot(balanceOfSlot, account);
      // assert
  ...
  describe("calculateAddressAllowanceSlots", () => {
    it("should calculate account balance storage slot", () => {
      const allowanceSlot = ethers.AbiCoder.defaultAbiCoder().encode(["uint"], [10]); // usdc's `allowance` mapping slot
      // https://dashboard.tenderly.co/paraswap/paraswap/simulator/e2a6cad1-4a82-4ba7-8c3e-ef097315542c/debugger?trace=0.2.0
      const expected = "0x371a34ab690ce2826b137a25bfdaf43d0b93c819878de2df8877f1fb50ffb206";
      const calculated = tenderly.calculateAddressAllowanceSlot(allowanceSlot, account, spender);
      // assert
      expect(calculated).toEqual(expected);
    });
  });
  ...
  describe("buildBalanceOfSimulationRequest", () => {
    it("should build `balanceOf` simulation request", () => {
      const request = tenderly.buildBalanceOfSimulationRequest(1, USDC, account);

      const expectedCallData = ethers.concat([
        "0x70a08231", // selector
  ...
  describe("buildAllowanceSimulationRequest", () => {
    it("should build `allowance` simulation request", () => {
      const request = tenderly.buildAllowanceSimulationRequest(1, USDC, account, spender);

      const expectedCallData = ethers.concat([
        "0xdd62ed3e", // selector
        ethers.AbiCoder.defaultAbiCoder().encode(["address", "address"], [account, spender]),
      ]);
      // assert
      expect(request.data).toEqual(expectedCallData);
    });
  ...
  describe("findTokenBalanceOfSlot", () => {
    it("should find Mainnet USDC `balanceOf` storage slot", async () => {
      const expectedSlot = ethers.AbiCoder.defaultAbiCoder().encode(["uint"], [9]);
    ...
    it("should find Mainnet USDT `balanceOf` storage slot", async () => {
      const expectedSlot = ethers.AbiCoder.defaultAbiCoder().encode(["uint"], [2]);
    ...
    it("should find Mainnet stETH `balanceOf` storage slot", async () => {
      const expectedSlot = ethers.AbiCoder.defaultAbiCoder().encode(["uint"], [0]);
    ...
    it("should find Mainnet aEthDai `balanceOf` storage slot", async () => {
      const expectedSlot = ethers.AbiCoder.defaultAbiCoder().encode(["uint"], [52]);
      const foundSlot = await tenderly.findTokenBalanceOfSlot(
        1,
  ...
  describe("findTokenAllowanceSlot", () => {
    it("should find Mainnet USDC `allowance` storage slot", async () => {
      const expectedSlot = ethers.AbiCoder.defaultAbiCoder().encode(["uint"], [10]);
    ...
    it("should find Mainnet USDT `allowance` storage slot", async () => {
      const expectedSlot = ethers.AbiCoder.defaultAbiCoder().encode(["uint"], [5]);
    ...
    it("should find Mainnet stETH `allowance` storage slot", async () => {
      const expectedSlot = ethers.AbiCoder.defaultAbiCoder().encode(["uint"], [1]);
    ...
    it("should find Mainnet aEthDai `allowance` storage slot", async () => {
      const expectedSlot = ethers.AbiCoder.defaultAbiCoder().encode(["uint"], [53]);
      const foundSlot = await tenderly.findTokenAllowanceSlot(
        1,
        "0x018008bfb33d285247A21d44E50697654f754e63", // Aave's `aEthDai`
      );
